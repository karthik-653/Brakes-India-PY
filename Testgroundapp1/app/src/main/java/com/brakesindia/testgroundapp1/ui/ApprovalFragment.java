...
import com.brakesindia.testgroundapp1.OrganizerActivity;
...
public class ApprovalFragment extends Fragment {
    ...
    ArrayList<String> plantsList, celltypesList, modelsList, processList, appplantList;
    ...
    private void setAction() {
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(spinner_a.getSelectedItem().toString().isEmpty()||
                        spinner_b.getSelectedItem().toString().isEmpty()||
                        spinner_c.getSelectedItem().toString().isEmpty()||
                        spinner_d.getSelectedItem().toString().isEmpty()||
                        spinner_e.getSelectedItem().toString().isEmpty()||
                        spinner_f.getSelectedItem().toString().isEmpty()){
    ...
    private void getPlant(String date){
        System.out.println("Inside trydate");
        Call<AppPlantResponse> call = ApiClient.getUserService().getdataApprovalPlant(date);
        System.out.println("Call statement done");
        call.enqueue(new Callback<AppPlantResponse>() {
            @Override
            public void onResponse(Call<AppPlantResponse> call, Response<AppPlantResponse> response) {
                if(response.isSuccessful()){
                    System.out.println("Response for trydate successful");
                    System.out.println("Date passed: "+date);
                    AppPlantResponse appPlantResponse = response.body();
                    if(appPlantResponse != null){
                        submit.setVisibility(View.VISIBLE);
                        List<String> plants_imp = appPlantResponse.getPlants();
                        plantsList.clear();
                        ...
                        clearL5();
                        System.out.println("App Plants: "+ plantsList);
                    }
                    else{
                        System.out.println("Null response getAppPlant" +response.code());
                    }
                }
                else{
                    System.out.println("No response getAppPlat" +response.code());
                }
            }

            @Override
            public void onFailure(Call<AppPlantResponse> call, Throwable t) {
    ...
    private void getUnitsData() {
        Call<UnitResponse> call = ApiClient.getUserService().getdataApprovalUnit(date, spinner_a.getSelectedItem().toString());
    ...
    private void getCellType(){
        Call<CellTypeResponse> call = ApiClient.getUserService().getdataApprovalCellType(date,
                spinner_a.getSelectedItem().toString(), Integer.parseInt(spinner_b.getSelectedItem().toString()));
        call.enqueue(new Callback<CellTypeResponse>() {
            ...
            public void onResponse(Call<CellTypeResponse> call, Response<CellTypeResponse> response) {
                System.out.println("Celltype body: "+response.code());
    ...
    private void getCellNumber(){
        Call<CellNumberResponse> call = ApiClient.getUserService().getdataApprovalCellNo(date,
                spinner_a.getSelectedItem().toString(),
                Integer.parseInt(spinner_b.getSelectedItem().toString()),
                spinner_c.getSelectedItem().toString());
    ...
    private void getModels(){
        ...
        Call<ModelResponse> call = ApiClient.getUserService().getdataApprovalModel(date,
                spinner_a.getSelectedItem().toString(),
                Integer.parseInt(spinner_b.getSelectedItem().toString()),
                spinner_c.getSelectedItem().toString(),
                Integer.parseInt(spinner_d.getSelectedItem().toString()));
    ...
    private void getProcess(){
        Call<ProcessResponse> call = ApiClient.getUserService().getdataApprovalProcess(date,
                spinner_a.getSelectedItem().toString(),
                Integer.parseInt(spinner_b.getSelectedItem().toString()),
                spinner_c.getSelectedItem().toString(),
                Integer.parseInt(spinner_d.getSelectedItem().toString()),
                spinner_e.getSelectedItem().toString());
    ...
    private void clearL6(){
        System.out.println("CLEAR L6 called");
        appplantList.clear();
        adapter_spinner_a.notifyDataSetChanged();
        unitsList.clear();
        adapter_spinner_b.notifyDataSetChanged();
    ...
    private void openDate(){
        DatePickerDialog datePickerDialog = new DatePickerDialog(requireContext(), R.style.datetheme, new DatePickerDialog.OnDateSetListener() {
            ...
            public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                ...
                System.out.println("Date from opendate function is: "+date);
                clearL6();
                getPlant(date);
            }
